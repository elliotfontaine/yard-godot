...
func get_current_version() -> String:
	...
	plugin_cfg.load("res://addons/yard/plugin.cfg")
...
func _on_UpdateCheck_request_completed(
	result: int,
	response_code: int,
...
func request_update_download() -> void:
	...
	if DirAccess.dir_exists_absolute("res://test/project/"):
		prints(
			"[YARD] Looks like you are working on the addon. You can't update the addon from within itself."
		)
	...
	for path in files:
		...
		if path.ends_with("/"):
			...
			var file: FileAccess = FileAccess.open(
				"res://addons/".path_join(new_file_path),
				FileAccess.WRITE,
			)
...
func _on_update_check_completed(result: int) -> void:
	var result_color: Color
	match result:
		UpdateCheckResult.UPDATE_AVAILABLE:
			result_color = version_indicator.get_theme_color("warning_color", "Editor")
			version_indicator.icon = version_indicator.get_theme_icon(
				"StatusWarning",
				"EditorIcons",
			)
